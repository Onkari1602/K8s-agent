        ...
        try:
            ...
            for n in nodes[:10]:
                node_details.append({
                    ...
                    "capacity": n.capacity_type,
                    "cpu_alloc": str(n.cpu_allocated_pct) + "%",
                    "mem_alloc": str(n.memory_allocated_pct) + "%",
                    "pods": n.pod_count,
                })

            node_json = json.dumps(node_details, indent=2)
            oversized_json = json.dumps(oversized[:5], indent=2)
            spot_json = json.dumps(spot_recs[:5], indent=2)
            rightsize_json = json.dumps(right_size[:5], indent=2)
